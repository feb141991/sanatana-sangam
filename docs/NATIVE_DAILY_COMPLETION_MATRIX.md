...
Status: Read-only audit (per `docs/NATIVE_APP_AGENT_RULES.md` default mode), with narrow P0 RPC-hardening exceptions applied directly per this task's own rule ("do not implement UI yet unless the fix is a clear P0 security issue") — no UI was touched. No application runtime code was changed by this audit.
...
Live native code (this repo, as of today) contradicts both: the bottom tab bar is Home/Japa/Bhakti/Pathshala/**Mandali** (Mandali now a primary tab, built to full parity including join flow, comments, RSVPs; Vichaar Sabha deferred), there is **no Kul screen anywhere in the native repo**, and Quiz/Nitya Karma/Dharm Veer/Vrat/Kosh already have real, committed implementations — well past "Phase 0 audit."
...
- **P0-1 — `increment_period_seva` had no ownership check, and the first fix missed the default `PUBLIC` execute surface. Fixed by two migrations.**
  Evidence: `supabase/public_schema.sql:473` (pre-fix definition, schema dump not yet regenerated), `supabase/migrations/20260708140920_harden_increment_period_seva.sql` (ownership check + anon revoke, already committed `e2ae927` before this audit), and `supabase/migrations/20260708153633_revoke_public_increment_period_seva_execute.sql` (explicit `PUBLIC` revoke added during this review).
  ...
  Status: Resolved once both migrations are applied — RPC now resolves `auth.uid()` internally, rejects mismatched `p_user_id`, clamps `p_points` to 1-500, and revokes both `PUBLIC` and `anon` execution before granting only `authenticated`/`service_role`. Migrations not yet applied to any live database from this sandbox (no local Postgres available) — needs `supabase db push` against the actual project.
...
1. Apply both P0-1 migrations (`20260708140920_harden_increment_period_seva.sql` and `20260708153633_revoke_public_increment_period_seva_execute.sql`) to the live database — not yet deployed from this sandbox (no local Postgres).
...
- supabase/migrations/20260708153633_revoke_public_increment_period_seva_execute.sql (written during review)
...
- No application UI code (native or web) was modified. The first P0-1 fix referenced above (`increment_period_seva`) was implemented and committed in a prior session task today, before this audit began; this review added the missing explicit `PUBLIC` execute revoke for the same RPC.
