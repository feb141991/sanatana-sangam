...
| 2.10 | **Vedic-day boundary rule (D4 partial)** | 🟡 (resolver ✅, diff report ❌ D28) | Layer C | Implemented `packages/panchang-engine/src/core/day-boundary.ts` (Version `1.0.0`) per §4 ahorātra rule. Per-user read-time resolver. Shadow Mode diff script `scripts/diff-vedic-day-boundary.ts` committed report `docs/VEDIC_DAY_BOUNDARY_DIFF_REPORT.md` across 2026–2028 (234 observances: 42.7% unchanged, 13.2% Ujjain boundary corrections, 44.0% diaspora locality shifts). Nothing wired in; `verify:calendar` 988/216 unchanged. |
...
| 3.2 | **Condition evaluator incl. prevalence + `viddha` (D4 partial)** | ⬜ **REJECTED — see D26/D27** | | Implemented `packages/dharma-rules/src/conditions/` (`CONDITION_EVALUATOR_VERSION = '1.0.0'`). Evaluates position, prevalence (`at`, `prevails`, `touches`, `majority`), muhurta/moonrise windows, and `viddha` conditions with detailed `reasons[]` and diagnostics. Adjudication script `scripts/adjudicate-conditions.ts` committed report `docs/ADJUDICATION_REPORT.md` (Shivaratri, Janmashtami dual-variant, Karva Chauth, Sankashti). Pure evaluator, not wired into engine. |
| 3.3 | `calendar_profiles` / `tradition_profiles` tables | ⬜ | | |
| 3.4 | **`observance_occurrences` variant qualification + uniqueness change (D15)** | 🟡 (no rollback; rule-7 question open) | variants | Migration `20260804030000_observance_occurrences_variant_qualification.sql`. Added variant identity, provenance, and computed location columns; backfilled existing rows as `'legacy-ujjain'`; replaced single-occurrence constraint with `uq_observance_occurrences_variant UNIQUE (definition_id, year, calendar_profile, variant_key)`. |
