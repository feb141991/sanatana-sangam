...
| **D29** | **Vaiṣṇava Janmāṣṭamī qualifies NO date in 2027 and 2028.** Strictly encoding `udaya-vyapini Ashtami AND Rohini at sunrise` fails when Kṛṣṇa Aṣṭamī and Rohiṇī nakṣatra do not coincide at the sunrise instant (e.g. 2027 Ujjain has Aṣṭamī on 25 Aug with Kṛttikā, and Rohiṇī on 26 Aug with Navamī). Traditional Gaudiya/Vaishnava texts (e.g. *Hari-bhakti-vilasa*) prescribe fallback precedence rules (observing on Ashtami-with-Rohini at night or Rohini-udaya when Ashtami is absent at sunrise). Classified as **(a)** doctrinal fallback requiring `[S]` ratification. | `packages/dharma-rules/src/festivals/rules.json` | **Open `[S]`** |
...
| 4.3 | Astronomical validation vs Tier-1 sources, 12 cities | 🟡 | `fae2b0f`. **Moonrise and sunrise validated at 13 sites against USNO (Tier 1)** — values read by a human, all inside the §10 two-minute tolerance. Also surfaced the systematic moonrise bias, since reduced 62% by the sidereal UT fix (`be19905`). **Remaining §10 quantities uncovered:** sunset, tithi boundary, nakshatra boundary, Sankranti instant, Nishita window. |
