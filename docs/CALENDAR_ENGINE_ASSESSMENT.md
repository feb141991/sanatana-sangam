...
| 3.8 | Alternatives + `reasons[]` in result contract | 🟡 | "Why today?" | `src/lib/calendar/observance-formatter.ts` builds the contract and groups variants. **NOT done and currently unsafe:** classification is by ROW COUNT alone (~line 210) — any festival with two rows becomes a variant pair, with no check that both days are traditionally observed. Latent today (Shivaratri has one stored row); becomes live the moment `USE_CONDITION_EVALUATOR` flips, when the evaluator's 15 Feb would be published beside the locked 17 Feb as a legitimate variant. Needs the dispute / uncertainty / error three-way gate first. |
