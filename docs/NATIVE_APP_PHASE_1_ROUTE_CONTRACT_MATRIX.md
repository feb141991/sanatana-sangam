...
| `app/(tabs)/pathshala.tsx` | `/pathshala` | API (`/api/pathshala/paths`) + Supabase progress read | `guided_path_progress` upsert on enrollment | User-owned write is allowed by RLS; enrollment should still receive a future contract review | Browse data no longer uses native `SEED_PATHS`; local type import remains | Browse migrated; enrollment write pending contract review | P1 |
| `app/pathshala/[pathId].tsx` | `/pathshala/[id]` | Supabase direct | None | Direct reads bypass central logic | None | Replace with API | P1 |
| `app/pathshala/[pathId]/[lessonId].tsx` | `/pathshala/[pathId]/[lessonId]` | Dead API call plus Supabase fallback | `guided_path_progress` upsert | Calls missing `/api/pathshala/progress`, then falls back to user-owned direct write allowed by RLS | Progression logic and local lesson data | Create `/api/pathshala/progress` and migrate native to fix data-drop bug | P1 |
...
- **P1**: Native Pathshala lesson detail references missing `/api/pathshala/progress`. DECISION: Create `/api/pathshala/progress` to centralize progress validation and daily/practice side effects. Migrate native to use it once the route exists, then migrate web while preserving current completion behavior.
