...
| `app/(tabs)/pathshala.tsx` | `/pathshala` | Supabase direct | `guided_path_progress` upsert | User-owned write; verify RLS and progression validation | None | Replace with API or shared package | P1 |
| `app/pathshala/[pathId].tsx` | `/pathshala/[id]` | Supabase direct | None | Direct reads bypass central logic | None | Replace with API | P1 |
| `app/pathshala/[pathId]/[lessonId].tsx` | `/pathshala/[pathId]/[lessonId]` | Mixed (API + Supabase direct) | `guided_path_progress` upsert | User-owned write; progression should be contract-backed | Progression logic | Replace with API or shared package | P1 |
...
|---|---|---|---|
| `lib/panchang.ts` | Duplicated Panchang math | Replace with `@sangam/panchang-engine` | P1 |
