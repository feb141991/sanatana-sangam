...
export interface SarvamProviderConfig {
  ...
  apiKey?: string;
  /** Sarvam model name override (default: sarvam-105b-conversations). */
  model?: string;
...
export class SarvamProvider implements PramanaInferenceProvider {
  ...
  private async _doGenerate(
    ...
    const payload: Record<string, unknown> = {
      model: this.config!.model ?? 'sarvam-105b-conversations',
      messages,
    ...
    return {
      text: generatedText,
      modelUsed: data.model ?? this.config!.model ?? 'sarvam-105b-conversations',
