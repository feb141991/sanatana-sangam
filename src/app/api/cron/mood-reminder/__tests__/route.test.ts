...
describe('GET /api/cron/mood-reminder', () => {
  ...
  it('rejects unauthorized requests', async () => {
    const req = new Request('http://localhost/api/cron/mood-reminder', {
  ...
  it('enqueues into notification_schedule under legacy mode', async () => {
    ...
    const upsertScheduleMock = vi.fn().mockReturnValue({
      select: vi.fn().mockResolvedValue({ data: [{ id: 'sched-1' }], error: null }),
    });
