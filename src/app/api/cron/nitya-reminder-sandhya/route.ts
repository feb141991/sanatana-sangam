...
import { produceSandhyaNityaCandidate } from '@/lib/nitya-candidate-producer';
  ...
  try {
    ...
    if (pipelineMode === 'candidate') {
      ...
      for (const u of users) {
        ...
        const candidate = produceSandhyaNityaCandidate(
          {
            id: u.id,
            ...
            wants_evening_reminder: u.wants_evening_reminder,
      ...
      for (let i = 0; i < candidateRowsToInsert.length; i += 100) {
        ...
        if (upsertErr) {
          console.error('[nitya-reminder-sandhya/candidate] Candidate upsert error:', upsertErr.message);
      ...
      return NextResponse.json({
        ok: true,
        pipeline_mode: 'candidate',
        message: 'Nitya sandhya candidates produced successfully',
        total_eligible: users.length,
        candidates_produced: candidateRowsToInsert.length,
        candidates_inserted: totalCandidates,
      });
