...
export async function GET(request: NextRequest) {
  ...
  // ── Mode 1: Detailed single story / definition review lookup ───────────────
  if (storyId || definitionId) {
    let targetStoryId = storyId;
    let targetDefinitionId = definitionId;

    if (!targetStoryId && targetDefinitionId) {
      const { data: latestVersion } = await db
        .from("observance_story_versions")
        .select("id")
        .eq("definition_id", targetDefinitionId)
        .order("version", { ascending: false })
        .limit(1)
        .maybeSingle();
      targetStoryId = latestVersion?.id ?? null;
    }
    ...
    if (targetStoryId && !targetDefinitionId) {
      const { data: storyRow } = await db
        ...
        .eq("id", targetStoryId)
        .single();
      targetDefinitionId = storyRow?.definition_id ?? null;
    }

    const [storyRes, allVersionsRes, translationsRes, sourcesRes, allSourcesRes, artworkRes, sharesRes, auditRes] = await Promise.all([
      targetStoryId
        ? db.from("observance_story_versions").select("*").eq("id", targetStoryId).single()
        : Promise.resolve({ data: null, error: null }),
      targetDefinitionId
        ? db.from("observance_story_versions").select("*").eq("definition_id", targetDefinitionId).order("version", { ascending: false })
        : Promise.resolve({ data: [], error: null }),
      targetStoryId
        ? db.from("observance_story_translations").select("*").eq("story_version_id", targetStoryId)
        : Promise.resolve({ data: [], error: null }),
      targetStoryId
        ? db.from("observance_story_source_links").select("*, observance_content_sources(*)").eq("story_version_id", targetStoryId)
        : Promise.resolve({ data: [], error: null }),
      targetDefinitionId
        ? db.from("observance_content_sources").select("*").eq("definition_id", targetDefinitionId).order("created_at", { ascending: false })
        : Promise.resolve({ data: [], error: null }),
      targetStoryId
        ? db.from("observance_artwork").select("*").eq("story_version_id", targetStoryId).order("created_at", { ascending: false })
        : Promise.resolve({ data: [], error: null }),
      targetStoryId
        ? db.from("observance_share_templates").select("*").eq("story_version_id", targetStoryId).order("language", { ascending: true })
        : Promise.resolve({ data: [], error: null }),
      targetDefinitionId
        ? db.from("observance_content_audit_log").select("*").order("created_at", { ascending: false }).limit(50)
        : Promise.resolve({ data: [], error: null }),
    ]);
  ...
  if (definitionsResult.error || migrationError) {
    return NextResponse.json({ error: "Observance content migration is not available", detail: (definitionsResult.error ?? migrationError).message }, { status: 503 });
  }
...
export async function POST(request: NextRequest) {
  ...
  if (body.action === "update_translations") {
    const { storyId, translations } = body;
    for (const t of (translations || [])) {
      await db.from("observance_story_translations").update({
        teaser: t.teaser,
        origin: t.origin,
        significance: t.significance,
        rituals: t.rituals,
        verse: t.verse,
        personal_practice: t.personal_practice,
        updated_at: new Date().toISOString(),
      }).eq("story_version_id", storyId).eq("language", t.language);
    }
    await audit(db, "translations_updated", "story_version", storyId, {});
    return NextResponse.json({ ok: true });
  }

  if (body.action === "update_share_template") {
    const { templateId, title, message, cta } = body;
    const { error } = await db.from("observance_share_templates").update({
      title,
      message,
      cta,
      updated_at: new Date().toISOString(),
    ...
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
    await audit(db, "share_template_updated", "share_template", templateId, {});
    return NextResponse.json({ ok: true });
  }
  ...
  if (body.action === "rollback") {
    const { storyId, definitionId } = body;
    const now = new Date().toISOString();
    ...
    const { error } = await db.from("observance_story_versions").update({ status: "published", published_at: now }).eq("id", storyId);
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
    await audit(db, "story_rolled_back", "story_version", storyId, { definitionId });
    return NextResponse.json({ ok: true });
  }
