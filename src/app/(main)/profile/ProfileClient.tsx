...
export default function ProfileClient({
  ...
  async function uploadAvatar(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      // Upload to Supabase Storage avatars bucket (upsert = replace existing)
      ...
      const displayUrl = `${signedData.signedUrl}&t=${Date.now()}`; // cache-bust

      // Also store the public URL in profiles for use elsewhere (members list etc.)
      const { data: pubData } = supabase.storage.from('avatars').getPublicUrl(path);
      const { error: dbError } = await supabase
        .from('profiles')
        .update({ avatar_url: pubData.publicUrl })
        .eq('id', userId);
