...
import type { StotramSummary } from '@/lib/stotrams';
...
interface BrowseInnerProps {
  stotramSummaries: StotramSummary[];
  deityMeta: DeityMeta;
  moodMeta: MoodMeta;
}

function BrowseInner({ stotramSummaries, deityMeta, moodMeta }: BrowseInnerProps) {
  ...
  const DEITIES = useMemo(() => Object.entries(deityMeta).map(([id, m]) => ({ id, ...m })), [deityMeta]);
  const MOODS   = useMemo(() => Object.entries(moodMeta).map(([id, m]) => ({ id, ...m })), [moodMeta]);
  ...
  const filtered = useMemo(() => {
    return stotramSummaries.filter(s => {
  ...
  }, [stotramSummaries, tradition, deity, mood, type]);
      ...
      <div className="px-4 space-y-3">
        {filtered.length === 0 ? (
          ...
          filtered.map((stotram, i) => {
            const deityInfo = deityMeta[stotram.deity] ?? deityMeta.universal;
            return (
              <motion.div key={stotram.id}
                initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
                transition={{ delay: i * 0.04, duration: 0.4 }}>
                <div className="rounded-[1.5rem] p-4 relative overflow-hidden"
                  style={{ background: cardBg, border: `1px solid ${deityInfo.color}22` }}>
                  {/* Color accent strip */}
                  <div className="absolute left-0 top-0 bottom-0 w-1 rounded-l-[1.5rem]"
                    style={{ background: deityInfo.color, opacity: 0.6 }} />
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          <p className="text-[11px] mt-0.5" style={{ color: deityInfo.color }}>{stotram.titleDevanagari}</p>
                        </div>
                        <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                          <span className="rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide"
                            style={{ background: `${deityInfo.color}18`, color: deityInfo.color }}>
                            {TYPE_LABELS[stotram.type] ?? stotram.type}
                          </span>
                          {stotram.verseCount > 1 && (
                            <span className="text-[9px]" style={{ color: textD }}>{stotram.verseCount} verses</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 mt-3">
                        <Link href={`/bhakti/stotram/${stotram.id}`}
                          ...
                          style={{ background: `${deityInfo.color}18`, border: `1px solid ${deityInfo.color}30`, color: deityInfo.color }}>
...
export default function BrowseClient(props: BrowseInnerProps) {
  return (
