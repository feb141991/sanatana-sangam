...
def score_gurbani_explain(result: dict[str, Any], case: dict[str, Any]) -> dict[str, Any]:
    ...
    if parsed_json:
        ...
        expected_keywords = []
        if "mantar" in case_id:
            expected_keywords = ["onkar", "creator", "one", "sat", "nam", "ओंकार", "ੴ", "ੴ", "ਕਰਤਾ"]
        elif "pauri" in case_id:
            expected_keywords = ["soch", "mind", "hukam", "will", "सोच", "ਹੁਕਮਿ", "ਸੋਚੈ"]
    ...
    if parsed_json:
        ...
        elif (expected_language in ["hi", "pa"]) and figure_theme_match:
            grounding_present = True
        elif len(parsed_json.get("meaning", "")) > 10:
            grounding_present = True

    # 4. Gurmat/Gurbani Commentary Quality
    ...
    if parsed_json:
        ...
        if expected_language == "hi":
            ...
            # English case: shouldn't be dominated by Devanagari or Gurmukhi
            devanagari_count = len(re.findall(r"[\u0900-\u097F]", meaning_text))
            gurmukhi_count = len(re.findall(r"[\u0A00-\u0A7F]", meaning_text))
            language_compliant = (devanagari_count + gurmukhi_count) < len(meaning_text) * 0.2
